...
import { Bell, X, Volume2, VolumeX, Zap, Cpu } from 'lucide-react';
...
export default function VibeLanding() {
  ...
  const commands: Record<string, { description: string; action: (args: string) => Promise<void> | void }> = {
    ...
    think: {
      description: "Toggle High Thinking mode for complex queries",
      action: () => {
        setIsThinkingMode(prev => {
          const next = !prev;
          setTerminalHistory(cur => [
            ...cur,
            { text: `think`, type: 'user' },
            { text: `High Thinking Mode: ${next ? 'ENABLED' : 'DISABLED'}. Switching model to ${next ? 'gemini-3.1-pro-preview' : 'gemini-3.5-flash'}.`, type: 'system' }
          ]);
          return next;
        });
      }
    ...
    concierge: {
      ...
      action: async (query) => {
        ...
        const processingMsg = isThinkingMode 
          ? "Deep Analysis Initialized. Accessing high-dimensional reasoning nodes..."
          : (interactionCount > 15 
        ...
        const geminiResponse = await sendMessageToGemini(query, isThinkingMode);
  ...
  const handleCommand = async (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter') {
      ...
      if (commands[lowerCmdName]) {
        ...
        const geminiResponse = await sendMessageToGemini(input, isThinkingMode);
            ...
            <div className="bg-[#1a1a1a] p-3 border-b border-[#333] flex justify-between items-center">
              ...
              <div className="flex gap-3 items-center">
                <button 
                  ...
                  className={`transition-colors flex items-center gap-1 ${isThinkingMode ? 'text-amber-glow' : 'text-gray-500 hover:text-white'}`}
                  title={isThinkingMode ? "High Thinking: Active" : "High Thinking: Inactive"}
                >
                  <Cpu className="w-4 h-4" />
                  {isThinkingMode && <motion.span initial={{ opacity: 0, x: -5 }} animate={{ opacity: 1, x: 0 }} className="text-[8px] font-bold">THINKING</motion.span>}
                </button>
                <button 
                  onClick={() => {
